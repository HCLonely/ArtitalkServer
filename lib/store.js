...
function createStore(databaseUrl = process.env.ARTITALK_DATABASE_URL) {
  ...
  async function ensureSchema() {
    ...
    // Register admin user from env vars if user table is empty
    const rows = await sql.query('SELECT count(*)::int AS count FROM artitalk_users');
    if (rows[0].count === 0) {
      const adminUsername = process.env.ADMIN_USERNAME;
      const adminPassword = process.env.ADMIN_PASSWORD;
      if (adminUsername && adminPassword) {
        const passwordRecord = await createPasswordHash(adminPassword);
        const id = objectId();
        ...
        await sql.query(
          ...
          [id, adminUsername, process.env.ADMIN_IMG || null, passwordRecord.password_hash, passwordRecord.password_salt, passwordRecord.password_algorithm, passwordRecord.password_iterations, now, now, JSON.stringify({})]
        );
      }
    }
  }
  ...
  async function migrateUsers(users) {
    ...
    for (const user of users) {
      ...
      try {
        const id = user.objectId || objectId();
        const now = user.createdAt ? new Date(user.createdAt).toISOString() : new Date().toISOString();
        const sourceData = { ...user };
        delete sourceData.objectId;
        delete sourceData.username;
        delete sourceData.createdAt;
        delete sourceData.updatedAt;
        await sql.query(
          `INSERT INTO artitalk_users (object_id, username, img, created_at, updated_at, source_data)
           VALUES ($1, $2, $3, $4, $5, $6::jsonb)
           ON CONFLICT (username) DO NOTHING`,
          [id, user.username, user.img || null, now, now, JSON.stringify(sourceData)]
        );
        imported++;
      } catch {
        // Skip individual user import failures
      }
    }
    return imported;
  }
