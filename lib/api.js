...
function readClassName(query) {
  const path = query.path || query.className || query[0];
  return Array.isArray(path) ? path[0] : path;
}

function readObjectId(query) {
  const path = query.path;
  if (Array.isArray(path)) return path[1];
...
function createClassHandler({ store }) {
  return async function classHandler(req, res) {
    ...
    try {
      const className = readClassName(req.query || {});
      const objectId = readObjectId(req.query || {});
