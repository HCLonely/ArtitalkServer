  ...
  return async function resetHandler(req, res) {
    ...
    try {
      ...
      if (!username || !legacyPassword || !newPassword) {
        return sendJson(req, res, 400, { error: 'username, legacyPassword, and newPassword are required' });
      }
      if (String(newPassword).length < 8) {
        return sendJson(req, res, 400, { error: 'newPassword must be at least 8 characters' });
      }
      ...
      if (!updated) return sendJson(req, res, 403, { error: 'Invalid reset proof' });
      return sendJson(req, res, 200, { ok: true, username });
    } catch (error) {
      return sendJson(req, res, error.statusCode || 500, { error: error.message });
    }
  };
